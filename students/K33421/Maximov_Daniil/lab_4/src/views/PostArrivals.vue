      ...
      <div class="add-postal-arrival-form text-center">
        ...
        <form @submit.prevent="addPostalArrival">
          <div class="form-group">
            <input
              v-model="postalArrivalForm.copies_received"
              type="number"
              placeholder="Количество поступивших копий"
              class="form-control"
              id="copiesReceived"
              required
            />
          ...
          <div class="form-group">
            <select
              v-model="postalArrivalForm.post_office"
              class="form-control"
              id="postOffice"
              required
            >
              <option disabled selected value="">Почтовое отделение</option>
          ...
          <div class="form-group">
            <select
              v-model="postalArrivalForm.newspaper"
              class="form-control"
              id="newspaperArrival"
              required
            >
    ...
    <template v-slot:list>
      <ul class="postal-arrival-list">
        <li v-for="postalArrival in postalArrivals" :key="postalArrival.id" class="list-group-item">
          ...
          <p>
            <strong>Почтовое отделение:</strong> {{ getPostOfficeNumber(postalArrival.post_office) }}
          </p>
          <p><strong>Газета:</strong> {{ getNewspaperName(postalArrival.newspaper) }}</p>
          <button @click="deletePostalArrival(postalArrival.id)" class="delete_button btn btn-danger">
            Удалить
          </button>
        </li>
      </ul>
