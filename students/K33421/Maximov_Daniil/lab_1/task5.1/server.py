...
class MyHTTPServer:
    ...
    def _serve_client(self, conn: socket.socket) -> None:
        ...
        if "/marks" in path:
            ...
            self._handle_marks_request(conn, subject)
    ...
    def _handle_get_request(self, conn: socket.socket) -> None:
        ...
        buttons = [
            f"<a href='/marks/{discipline}'>{discipline}</a>"
            for discipline in self._data.keys()
        ]

        formatted_buttons = "<br>".join(buttons)

        placeholder = "###PLACEHOLDER###"
        html_file = self._load_html_file("index.html")
        processed_html_file = html_file.replace(placeholder, formatted_buttons)

        self._send_response(conn, processed_html_file)

    def _handle_marks_request(self, conn: socket.socket, subject: str) -> None:
        marks = self._data.get(subject, [])
        response_content = f"<html><body><h1>Marks for {subject}</h1><p>{', '.join(marks)}</p></body></html>"
        self._send_response(conn, response_content)
